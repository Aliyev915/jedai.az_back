...
<div class="page">
   <div class="page-header">
      <h1 class="page-title">Tövsiyə Məktubları</h1>
   </div>
                  ...
                  <div class="example table-responsive">
                     ...
                     <table class="table table-bordered">
                        ...
                        <tbody>
                           ...
                           <tr align="center">
                              <td>{{$letter->sort}}</td>
                              <td><img src="{{ $letter->image}}" alt="{{ $letter->name_surname}}" width="75"></td>
                              <td><input class="status" data-id="{{$letter->id}}" type="checkbox" data-on="Aktiv" data-off="Passiv" 
                                 data-onstyle="success" data-offstyle="danger" @if($letter->status==1) checked @endif 
                                 data-toggle="toggle">
                              </td>
                              <td>
                                 <a data-id="{{$letter->id}}" class="btn btn-outline-primary edit-click"><i class="fa fa-edit pri"></i></a>
                                 <a data-id="{{$letter->id}}" class="btn btn-outline-danger delete-click"><i class="fa fa-trash dan"></i></a>
                              </td>
                           </tr>
                           @endforeach
                        </tbody>
...
<script>
   $(function() {
      ...
      $('.edit-click').click(function(){
         ...
         $.ajax({
            type:'GET',
            url:'/admin/letters/'+id+'/edit',
            success:function(data){
               console.log(data);    
               $("#editForm").attr({action: '/admin/letters/'+id});
               $('#sort').val(data.sort); 
      ...
      $('.status').change(function() {
         ...
         $.get("{{route('admin.letters.status')}}",{id:id,status:status}, function(data, status){
         console.log(data);
         });
      });
      ...
      $('.delete-click').click(function() {
         id = $(this)[0].getAttribute('data-id');
         $("#deleteForm").attr({action: '/admin/letters/'+id});
      ...
      <form method="POST" id="deleteForm" class="modal-content" enctype="multipart/form-data">
         ...
         <div class="modal-content">
            <div class="modal-header">
               ...
               <h4 class="modal-title">Tövsiyə Məktubunun Silinməsinə Əminsinizmi?</h4>
            </div>
...
<div class="modal fade" id="exampleFormModal" aria-hidden="false" aria-labelledby="exampleFormModalLabel"
   ...
   <div class="modal-dialog modal-simple">
      <form  method="POST" action="{{ route('letters.store')}}" class="modal-content" enctype="multipart/form-data">
         @csrf
         @method('POST')
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
