...
<div class="page">
   <div class="page-header">
      <h1 class="page-title">Komandamız</h1>
   </div>
                  ...
                  <div class="example table-responsive">
                     ...
                     <table class="table table-bordered" id="datatable-buttons">
                        ...
                        <tbody>
                           ...
                           <tr align="center">
                              <td>{{$team->sort}}</td>
                              <td><img src="{{ asset($team->image)}}" alt="{{ asset($team->name_surname)}}" width="75"></td>
                              <td>{{$team->name_surname}}</td>
                              <td>{{$team->position}}</td>
                              <td><input class="status" data-id="{{$team->id}}" type="checkbox" data-on="Aktiv" data-off="Passiv" 
                                 data-onstyle="success" data-offstyle="danger" @if($team->status==1) checked @endif 
                                 data-toggle="toggle">
                              </td>
                              <td>
                                 <a data-id="{{$team->id}}" class="btn btn-outline-primary edit-click"><i class="fa fa-edit pri"></i></a>
                                 <a data-id="{{$team->id}}" class="btn btn-outline-danger delete-click"><i class="fa fa-trash dan"></i></a>
                              </td>
                           </tr>
                           @endforeach
                        </tbody>
...
<script>
   $(function() {
      ...
      $('.edit-click').click(function(){
         ...
         $.ajax({
            ...
            success:function(data){
               console.log(data);    
               $("#editForm").attr({action: '/admin/team/'+id});
               $('#lang').val(data.lang);   
      ...
      $('.status').change(function() {
         ...
         $.get("{{route('admin.team.status')}}",{id:id,status:status}, function(data, status){
         console.log(data);
         });
      });
      ...
      $('.delete-click').click(function() {
         id = $(this)[0].getAttribute('data-id');
         $("#deleteForm").attr({action: '/admin/team/'+id});
